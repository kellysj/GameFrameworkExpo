...
public class Reed_Actor {
	...
	Reed_Actor(String imageFrames, String filename) throws FileNotFoundException {
		commands = new Scanner(new FileReader(filename));
		//StandbyActorNONE_1
		if(imageFrames.matches("^\\w*_\\d$")){
			System.out.println("special trigger");
			String [] temp = new String[2];
			temp = imageFrames.split("_");
			String imageName = temp[0];
			System.out.println("Reed_Actor imageName: " + imageName + "\nSpecial Check: " + temp[1]);
			...
			for(int i = 0; i < numFrames; i++){
				...
				frames = new EZImage[1];
				frames[0] = EZ.addImage(imageFrames, 0, 0);
				numFrames = standbyFrames = 1;
				group = EZ.addGroup();
				frames[0].hide();
				group.addElement(frames[0]);
			}
