...
public class animator {
	...
	public void sand_run()throws IOException{
		...
		sun = new actor( EZ.getWindowWidth()/10,(9*EZ.getWindowHeight()/10),0,1,"sun.png","kelly_script4.txt",(double)0.05);
		tree_test = new tree_actor(EZ.getWindowWidth()/2,9*(EZ.getWindowHeight()/10), "bark_small.png","leaf.png","orange.png",true);
		user_test = new user(1500, 900, 0,1,"muscle_torso.png","Kelly_script2.txt");
		background = new actor(EZ.getWindowWidth()/2,EZ.getWindowHeight()/2, 0,1,"space_background.png","kelly_script3.txt");
		while(true){
			ticker.update();
			user_test.update();
			//test.update();
