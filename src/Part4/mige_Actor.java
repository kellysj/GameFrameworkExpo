...
public class mige_Actor {
	...
	mige_Actor(String imageFrames, String filename) throws FileNotFoundException {
		commands = new Scanner(new FileReader(filename));
		String [] temp = new String[2];
		temp = imageFrames.split("_");
		String imageName = temp[0];
		standbyFrames = Integer.parseInt(temp[1]);
		...
		for(int i = 0; i < numFrames; i++){
			frames[i] = EZ.addImage(standbyFilenames[i], -200, -200);
			frames[i].hide();
			frames[i].translateTo(0, 0);
		...
		switch(command.name()){
			case "CONTROL":
				...
				if(command.duration() == 0){
					group.translateTo((float)Integer.parseInt(parameters[0]), (float)Integer.parseInt(parameters[1]));
					group.rotateTo((double)Double.parseDouble(parameters[2]));
					group.scaleTo((double)Double.parseDouble(parameters[3]));
					break;
				}
