...
public class tree_actor {
	...
	tree_actor(int x, int y, String bark_image_name,String leaf_image_name, String fruit_image_name, boolean image_on, boolean limb_trim,boolean leaf_on){
		id_num = 0;
		x_base = x;
		y_base = y;
		start_length = 100;
		...
		trim_limbs = limb_trim;
		leaves_on = leaf_on;
		this.tree_init();
	}
	public static void main(String args[]){
		EZ.initialize(2000, 1400);
		EZ.setBackgroundColor(Color.black);
		tree_actor tree_test = new tree_actor(EZ.getWindowWidth()/2,9*(EZ.getWindowHeight()/10), "bark_small.png","leaf.png","orange.png",false,false, false);
		while(true){
			tree_test.go();
			EZ.refreshScreen();
		}
